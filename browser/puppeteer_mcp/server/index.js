...
/**
 * Ensures a Puppeteer browser and page instance are available.
 * Handles restarting the browser if launchOptions change.
 * Validates dangerous arguments based on allowDangerous flag.
 * @param {object} args - Arguments containing launchOptions and allowDangerous.
 * @returns {Promise<import('puppeteer').Page>} The Puppeteer Page object.
 */
...
/**
 * Deeply merges two objects. For arrays in 'args' or 'ignoreDefaultArgs',
 * it deduplicates, preferring values from the source.
    ...
    if (Array.isArray(targetVal) && Array.isArray(sourceVal)) {
      ...
      output[key] = [...new Set([
        ...(key === 'args' || key === 'ignoreDefaultArgs' ?
          targetVal.filter((arg) => !sourceVal.some((launchArg) => typeof arg === 'string' && typeof launchArg === 'string' && arg.startsWith('--') && launchArg.startsWith(arg.split('=')[0]))) :
          targetVal),
        ...sourceVal
      ])];
    } else if (sourceVal instanceof Object && key in target && targetVal instanceof Object) {
...
/**
 * Handles the execution logic for different Puppeteer tools.
 * @param {string} name - The name of the tool to call.
 * @param {object} args - The arguments for the tool.
 * @returns {Promise<object>} A promise resolving to the CallToolResult object.
 */
    ...
    case "puppeteer_click":
      try {
        ...
        return {
          content: [{
            type: "text",
            text: `Failed to click ${args.selector}: ${error.message}`, // Cast removed
          }],
          isError: true,
        };
      }
    ...
    case "puppeteer_fill":
      try {
        ...
        return {
          content: [{
            type: "text",
            text: `Failed to fill ${args.selector}: ${error.message}`, // Cast removed
          }],
          isError: true,
        };
      }
    ...
    case "puppeteer_select":
        try {
            ...
            return {
                content: [{
                    type: "text",
                    text: `Failed to select ${args.selector}: ${error.message}`, // Cast removed
                }],
                isError: true,
            };
        }
    ...
    case "puppeteer_hover":
        try {
            ...
            return {
                content: [{
                    type: "text",
                    text: `Failed to hover ${args.selector}: ${error.message}`, // Cast removed
                }],
                isError: true,
            };
        }
      ...
      try {
        await page.evaluate(() => {
          ...
          ['log', 'info', 'warn', 'error'].forEach(method => {
            console[method] = (...args) => {
              window.mcpHelper.logs.push(`[${method}] ${args.map(arg => String(arg)).join(' ')}`);
        ...
        return {
          content: [{
            type: "text",
            text: `Script execution failed: ${error.message}`, // Cast removed
          }],
          isError: true,
        };
      }
...
// --- Server Execution ---
const isMainModule = import.meta.url.endsWith(process.argv[1]) || process.argv[1].endsWith('node');

if (isMainModule) {
    runStdioServer().catch(err => {
        console.error("Failed to run stdio server:", err);
        process.exit(1);
    });

    process.stdin.on("close", () => {
        console.error("STDIN closed, closing Puppeteer MCP Server");
...
let sseTransportInstance = null;

app.get("/sse", (_req, res) => {
  
  const transport = new SSEServerTransport("/messages", res); 
  sseTransportInstance = transport; 
  ...
  res.on('close', () => {
      ...
      if (transport === sseTransportInstance) {
...
app.post("/messages", (req, res) => {
  ...
  if (sseTransportInstance) {
    sseTransportInstance.handlePostMessage(req, res).catch(err => {
      ...
      res.status(400).send("No active SSE session for messages.");
  }
});

const port = process.env.SERVER_PORT || 3000;
app.listen(port, () => {
  console.error(`SSE transport server listening on http://localhost:${port}`);
});